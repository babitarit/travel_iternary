...
def save_to_pdf(itinerary_text):
    ...
    pdf_file_path = os.path.join(base_dir, "Travel_Itinerary.pdf")
    
    pdf.output(pdf_file_path)
    
    return pdf_file_path

last_itinerary = {"text": None}
...
def download_pdf():
    ...
    if not last_itinerary["text"]:
        return jsonify({"error": "No itinerary has been generated yet. Please generate an itinerary first."}), 404

    try:
        pdf_path = save_to_pdf(last_itinerary["text"])  # Generate PDF dynamically here
